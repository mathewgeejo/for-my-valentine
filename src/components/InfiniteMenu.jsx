...
class InfiniteGridMenu {
  ...
  #initDiscInstances(count) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      m.set(mat4.create()); this.discInstances.matrices.push(m);
    }
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketchRef.current = new InfiniteGridMenu(
        canvas, items.length ? items : defaultItems,
        handleActiveItem, setIsMoving, sk => sk.run(), scale
      );
    }
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          <h2
            className={`
              select-none absolute font-black text-[2rem] sm:text-[3rem] md:text-[4rem]
          ...
          <p
            className={`
              select-none absolute max-w-[10ch] text-[1rem] sm:text-[1.5rem]
